...
- `prop`을 직접 변경하는 상황 예)
  - `prop` 은 초기값만 절단하고, 자식 컴포넌트는 그 초기값을 로컬 데이터 속성으로 활용하고 싶은 경우
  ```
  props: ['initialCounter'],
  data: function () {
    return {
      counter: this.initialCounter0
    }
  }
  ```
...
- `prop` 이 아닌 속성은 컴포넌트에 전달되긴 하나 대응되는 `prop` 이 정의되지 않는다.
- 컴포넌트 라이브러리를 만드는 등의 경우 어떤 맥락에서 해당 컴포넌트가 사용될지를 확실히 상정할 수 없는 경우가 있다.
...
- 대부분의 속성의 경우 컴포넌트에 제공된 값은 컴포넌트에서 설정된 값을 대체한다.
- 위 예시 경우엔 `form-control date-pickef-theme-dark` 로 병합된다.

<br/>

##### `v-in` 을 이용한 사용자 지정 이벤트

- 부모가 `prop` 을 사용하여 자식에게 데이터를 전달하닥 문제가 발생했을 때, 부모에게 알리기 위한 정의 이벤트 시스템
- 모든 Vue 인스턴스는 아래와 같은 이벤트 인터페이스를 구현한다.
  - `$on(eventName)` 을 사용하여 이벤트를 감시하십시오.
...
- 부모 컴포넌트가 신경 쓸 수 있느 ㄴ경우를 대비하여 자체 활동에 대한 정보를 보고하는 것 뿐이다.
